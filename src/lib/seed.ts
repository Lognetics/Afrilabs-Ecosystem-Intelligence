...
export async function runSeed(prisma: PrismaClient) {
  ...
  const [admin, , ed, partnershipsLead, programsLead, membersLead] = users;
  ...
  for (let i = 0; i < HUB_DATA.length; i++) {
    ...
    if (i % 4 !== 0) {
      ...
      await prisma.hubPayment.create({
        data: { hubId: h.id, amount: tierAmt, status: "PAID",
      ...
      await prisma.hubPayment.create({
        data: { hubId: h.id, amount: tierAmt, status: i % 2 ? "OVERDUE" : "PENDING",
  ...
  const events = await Promise.all([
    ...
    prisma.event.create({
      data: {
        ...
        location: "Online", registered: 200, attended: 0,
  ...
  for (const [title, dept, status] of TASKS) {
    await prisma.task.create({ data: { title, department: dept, status, eventId: events[0].id, priority: "HIGH" } });
  }
  ...
  await prisma.campaign.create({ data: {
    ...
    sentAt: new Date(), createdById: membersLead.id,
  }});
  await prisma.campaign.create({ data: {
    ...
    scheduledAt: new Date(Date.now() + 1000 * 60 * 60 * 48), createdById: membersLead.id,
  }});
  await prisma.campaign.create({ data: {
  ...
  return {
    hubs: hubs.length, partners: partners.length, programs: programs.length,
    events: events.length, partnerships: partnerships.length, users: users.length,
  };
}
